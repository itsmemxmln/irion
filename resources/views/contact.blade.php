...
@if (app()->getLocale() == "de")
    @section('description','Treten Sie mit uns in Kontakt!')
@else
    @section('description','Contact us! We are constantly working on pushing technological boundaries. There is no run-of-the-mill with us.')
@endif
...
<section class="page-banner" style="background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.3)), url('{{ url('') }}/assets/img/home/oberflaechen/2.webp');">
    <h1 class="text-center text-white fw-bold animate__animated animate__zoomIn">{{ __("Kontakt")}}</h1>
</section>
...
{{-- <section>
    <div id="map"></div>
    <script
      src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCXL4v2nKeYrAR6d_HTwSClpTAMtuTvg7M&callback=initMap&libraries=&v=weekly"
      async
    >
    </script>
    <script>
        function initMap() {
            const position = { lat: 48.68049621254011, lng: 8.728079305756493 };
            const map = new google.maps.Map(document.getElementById("map"), {
                zoom: 9,
                center: position,
                mapId: '0',
                icon: 'https://preview1.suitec.de/assets/img/map-logo.png'
            });
